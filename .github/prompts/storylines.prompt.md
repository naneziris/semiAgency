...
1. Read `state.json`. If `kind` is not `deck`, stop: proposals use `/proposal`. If `gates.G1.passed` is false, stop: the facts in `content.md` must be checked first (gate G1); in the terminal, `python scripts/status.py` shows the checklist and the command that passes it.
...
5. Stop. Give your recommendation in 3 lines. Next, in the terminal: `python scripts/status.py --pass` (it asks S1 or S2).
