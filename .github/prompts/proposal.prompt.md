...
1. Read `state.json`. If `kind` is not `proposal`, stop: decks use `/storylines`. Use `gates.G2.chosen_option`; if missing, stop: an option must be chosen first (gate G2). In the terminal: `python scripts/status.py --pass`, which asks which option.
2. Read `discovery.md`, `options.md`, and `brand/voice.md` if present.
3. Write `proposal.md` per `.github/instructions/proposal.instructions.md`. Run `python scripts/validate.py <dir>/proposal.md` and fix errors, especially traceability gaps.
...
5. Stop. Summarize: the 3 proposal claims you are least sure about (gate G3 check) and your storyline recommendation in 3 lines. Next, in the terminal: `python scripts/status.py --pass` (it asks S1 or S2).
