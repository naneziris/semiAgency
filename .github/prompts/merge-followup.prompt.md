...
---
Engagement: `${input:engagement}`; if that is empty, use the one line in `engagements/CURRENT`. `<dir>` = `engagements/<that name>`. New material: `${input:file}` (a path under inputs/). Agent: `@analyst`.
1. Run `python scripts/ingest.py <dir>` so the new file is normalized and has a source id.
...
5. Run `python scripts/followup_agenda.py <dir>` and tell the user whether another follow-up is needed (any blocking Q still open), then to re-pass G1 with `python scripts/status.py --pass G1`.
