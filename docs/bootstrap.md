...
| `new_engagement.py` | `--kind proposal\|deck\|meeting`, `--name`, `--subject`, `--stakeholders`, `--audience`, `--minutes`, `--from <dir>`; **with no `--kind` it asks the three questions from README.md "Which runbook?" interactively and derives the kind** | folder tree, `state.json` (kind, stage 0, all gates false), the templates that kind needs, plus `templates/audience.md` → `inputs/audience.md` for proposal and deck kinds; `--from` copies another engagement's `inputs/` and `normalized/`; ends by printing the runbook path and step 0 | idempotent; refuses to overwrite |
...
| `append_tasks.py` | engagement dir | rows appended to `tracker/actions.csv` | **ships** — stdlib csv, dedup on sha1(source, engagement, action); Phase 5 only extends validate.py for tasks.json |
...
Implement scripts/new_engagement.py (interactive kind questions when --kind is omitted), scripts/ingest.py, scripts/validate.py (schemas for brief.md, discovery.md, content.md, notes.md for now), scripts/status.py (kind-aware gate map; prints the current `## Step N` section of docs/runbooks/<kind>.md), scripts/followup_agenda.py per the specifications table in docs/bootstrap.md §2.
...
```
Implement scripts/build_deck.py, scripts/storyboard.py, scripts/lint_deck.py per docs/design.md §5.2 and docs/bootstrap.md §2, using ooxml.py.
...
`docs/runbooks/proposal.md`, `deck.md`, `meeting.md` — one page each. `python scripts/status.py <dir>` prints the current step.
