...
| `append_tasks.py` | reviewed tasks (`tasks.json` of an engagement, or any `*.todos.json`) → tracker, deduplicated; lock → `tasks.pending.csv` |
| `kb_ingest.py` | `kb/inbox/*` handoffs → routed into `kb/` (docs/handoff-format.md) |
| `kb_index.py` | rebuild `kb/index.md` and `kb/topics/<t>/links.md` |
| `brief_collect.py` | today's calendar + tracker + triage/digest status + 1-1 history + engagement gates → `kb/briefs/<date>.inputs.md` |
